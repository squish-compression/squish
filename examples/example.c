 ...
 * Run:  ./examples/example [archive.sqsh]
 */
...
int main(int argc, char **argv) {
    ...
    int rc = squish_compress_alloc(text, n, &comp, &comp_len, 1, 0, NULL, NULL);
    if (rc != SQUISH_OK) {
        fprintf(stderr, "compress: %s\n", squish_strerror(rc));
        return 1;
    }
    printf("compressed %zu -> %zu bytes (a one-member archive)\n", n, comp_len);

    void *back; size_t back_len;
    rc = squish_decompress_alloc(comp, comp_len, &back, &back_len, 1, NULL, NULL);
    if (rc != SQUISH_OK) {
